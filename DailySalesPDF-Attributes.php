...
function Header()
{
   ...
   $this->SetFills(array(true,true,true,true,true,true,true,true,true,true, true, true, true));

   $this->SetWidths(array(4.4,3,3,2,1.48,2,2,2.4,2,2,2,2,2.2));
   $this->SetAligns(array('C','C','C','C','C','C','C','C','C','C','C','C','C'));
   $this->Ln(3);
   $this->Row(array("NAME OF\nCONTRACTOR","LABORATORY\nNUMBER", "KINDS OF\nTESTING", "UNIT\nPRICE", 'QTY', "AMOUNT", "GROSS\nSALES", "%REBATES", "W/ TAX", "AMOUNT\nPAID", "O.R. #", "BILLING\nNUMBER", "REMARKS"));
}
...
{   
    ...
    // Page number
    $this->Cell(0,10,'MTCI-DVO DAILY SALES REPORT '.$date,0,0,'L');
